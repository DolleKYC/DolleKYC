...
const products = [
    {
        id: 1,
        name: "Bybit KYC",
        description: "Verification KYC for Bybit platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 2,
        name: "MEXC KYC",
        description: "Verification KYC for MEXC platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 3,
        name: "OKX KYC",
        description: "Verification KYC for OKX platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 4,
        name: "BITGET KYC",
        description: "Verification KYC for BITGET platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 5,
        name: "KUCOIN KYC",
        description: "Verification KYC for KUCOIN platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 6,
        name: "BINGX KYC",
        description: "Verification KYC for BINGX platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 7,
        name: "BINANCE KYC",
        description: "Verification KYC for BINANCE platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 9,
        name: "WEEX KYC",
        description: "Verification KYC for WEEX platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        id: 10,
        name: "KCEX KYC",
        description: "Verification KYC for KCEX platform, fast and secure process for your digital asset trading. This service ensures your account is fully verified according to global standards.",
    ...
    },
    {
        ...
        mailAccountOptions: [
            { name: "Hotmail (outlook)", price: 0.30, descriptionDetails: "Product: Mails Hotmail(outlook)\nMail:password\nPrice per item: 0.30$\nMinimum quantity: 10" },
            { name: "Gmail", price: 0.80, descriptionDetails: "Product: Mails Gmail\nIssue format login:password:recovery email\nAccounts with stale, confirmed with SMS.\nGender can be male or female\nPrice per unit: 0.80$\nMinimum quantity: 10" },
            { name: "Firstmail", price: 0.10, descriptionDetails: "Product: Mails Firstmail\nMail login site firstmail.ltd\nIssue format login:password\nImap enabled\nPrice per unit: 0.10$\nMinimum quantity: 10" },
            { name: "Rambler", price: 0.10, descriptionDetails: "Product: Mails Rambler\nLogin:password\nPrice per unit: 0.10$\nMinimum quantity: 10" }
        ],
...
function displayProducts(filteredProducts) {
    ...
    if (filteredProducts.length === 0) {
        productGrid.innerHTML = `<p class="col-span-full text-center text-gray-400 text-lg">No products found.</p>`; // Static English text
        return;
    }
...
function openProductModal(product) {
    ...
    modalSelectedPrice.textContent = 'Select country & product type'; // Static English text

    if (product.id === 11) {
        countrySelectLabel.textContent = 'Mail Accounts:'; // Static English text
        productTypeSection.classList.add('hidden');
        mailServiceDescriptionSection.classList.remove('hidden');

        countrySelect.innerHTML = '<option value="" disabled selected>Select mail account type</option>'; // Static English text
        if (product.mailAccountOptions && product.mailAccountOptions.length > 0) {
            ...
            countrySelect.innerHTML = '<option value="" disabled selected>No mail account types available</option>'; // Static English text
        ...
        countrySelectLabel.textContent = 'Select Country for KYC:'; // Static English text
        productTypeSection.classList.remove('hidden');
        mailServiceDescriptionSection.classList.add('hidden');

        countrySelect.innerHTML = '<option value="" disabled selected>Select country</option>'; // Static English text
        if (product.countryPrices && product.countryPrices.length > 0) {
            ...
            countrySelect.innerHTML = '<option value="" disabled selected>No countries available</option>'; // Static English text
            countrySelect.disabled = true;
            countrySelect.classList.add('opacity-50', 'cursor-not-allowed');
            selectedCountryData = null;
        }

        productTypeSelect.innerHTML = '<option value="" disabled selected>Select product type</option>'; // Static English text
        if (product.productTypes && product.productTypes.length > 0) {
            ...
            productTypeSelect.innerHTML = '<option value="" disabled selected>No product types available</option>'; // Static English text
...
function updateFinalPriceAndButtonState() {
    if (currentProduct.id === 11) {
         if (selectedCountryData) {
            ...
            modalSelectedPrice.textContent = 'Select mail account type'; // Static English text
...
addToCartModalBtn.addEventListener('click', () => {
    ...
    if (currentProduct && selectedCountryData && (currentProduct.id === 11 || selectedProductTypeData)) {
        ...
        if (currentProduct.id === 11) {
            alertUser("Please select mail account type first."); // Static English text
        } else {
            alertUser("Please select country and product type first."); // Static English text
        }
    }
});
...
function addToCart(product, countryData, productTypeData, itemPrice, quantity) {
    ...
    alertUser(`${quantity} ${product.name} for ${countryData.country} with ${productTypeData.name} added to cart!`); // Static English text
}
    ...
    if (cart.length === 0) {
        ...
        cart.forEach(item => {
            ...
            cartItemDiv.innerHTML = `
                ...
                <button class="remove-all-item-btn bg-red-800 hover:bg-red-900 text-white text-xs font-bold py-1 px-2 rounded-lg transition-all duration-300 ml-2" data-unique-id="${item.uniqueId}">Remove</button> <!-- Static English text -->
            `;
...
function clearCart() {
    ...
    updateCartCount();
    renderCartItems();
    alertUser("Cart has been cleared!"); // Static English text
...
checkoutButton.addEventListener('click', () => {
    if (cart.length === 0) {
        alertUser("Your cart is empty. Please add items to proceed."); // Static English text
        return;
    }
...
payWithTelegramBtn.addEventListener('click', () => {
    ...
    const telegramUrl = `https://t.me/kyctime?text=${encodedMessage}`;
